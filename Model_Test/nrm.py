...
def main():
    ...
    for vnr in range(len(vne_list)):
        ...
        temp = []
        total_vm_request += vne_list[vnr].nodes
        ...
        temp = []
        total_vl_request += len(vne_list[vnr].edges)
    ...
    for req_no in req_order:
        ...
        if not edge_map(substrate, vne_list[req_no], req_no, req_map, vne_list):
            ...
            continue
        accepted += 1
        total_vm_embedded += len(vne_list[req_no].node_weights)
        total_vl_embedded += len(vne_list[req_no].edge_weights)//2
    ...
    average_node_stress = 0
    num_vnodes = 0
    for req in curr_map.values():
        num_vnodes += len(req.node_map)
    count = len(curr_map)
    snodes = len(substrate.node_weights)
    average_node_stress = num_vnodes/(snodes*count)

    average_link_stress = 0
    vlinks = slinks = 0
    for req in curr_map.values():
        vlinks += len(req.edge_map)
    slinks = len(substrate.edge_weights)//2
    average_link_stress = vlinks/(slinks*count)
    ...
    avg_congestion = average_edge_utilization/(len(substrate.edge_weights)//2)
    if utilized_links != 0:
    ...
    output_dict = {
        ...
        "total_vl_request": total_vl_request,
        "total_vm_embedded":total_vm_embedded,
        "total_vl_embedded":total_vl_embedded,

        "avg_node_stress": average_node_stress,
        "avg_link_stress": average_link_stress,
