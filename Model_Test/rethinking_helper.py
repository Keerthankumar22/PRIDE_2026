...
def check_compatibility(chromosome, substrate_copy, virtual):
    for i, path in enumerate(chromosome.edge_map):
        for j in range(1, len(path)):
            ...
            print("Edge = ",edge)
            print("substrate copy edge weights is ",substrate_copy.edge_weights[edge])
            if (
                ...
                substrate_copy.edge_weights[(str(path[j - 1]), str(path[j]))]-= virtual.edge_weights[virtual.edges[i]]
