...
class ActorCritic(nn.Module):
    ...
    def __init__(self,num_sub):
        ...
        self.lrelu = nn.LeakyReLU(negative_slope=0.01, inplace=False)
        self.virt_graph = torch.zeros(5,4)
    ...
    def forward(self,state,j):
        ...
        # print("Virt graph is ",self.virt_graph)  
...
def gcnrl():
    ...
    total_vm_request = 0
    total_vl_request = 0    
    vne_list = sorted(vne_list,key=lambda vne:sum(vne.node_weights.values()))
    ...
    actor_critic_model.load_state_dict(torch.load('normal_gcn_rl_proposed.pth'))
    ...
    for i in range(req_no):
        ...
        sorted_vnodes = dict(sorted(vne_list[i].node_weights.items(), key=lambda item: item[1]))
        vm_count = 0
        for j in sorted_vnodes.keys():
        # for j in range(len(state[1].x)):
            probs, _ = actor_critic_model(state,j)      # actor prob, critic value
            ...
            if not vnom_done:
                ...
                break
            if i<req_no-1 and vm_count==len(state[1].x)-1:
                next_state = environment.get_state(substrate,vne_list[i+1])
            state = next_state.copy()
            vm_count += 1
        # print()      
    ...
    avg_link *= 100
    avg_node *= 100
    # print("Revenue to cost is ",rev/cost)
    if cost!=0:
        r2c = (rev/cost)*100
    else:
        r2c = 0
