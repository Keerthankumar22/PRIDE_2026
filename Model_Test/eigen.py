...
class FeatureMatrixSubstrate:
    ...
    def calc_eigen(self):
        ...
        for i in range(self.sub.nodes):
            ...
            for nd in self.sub.neighbours[i]:
                ls.append(int(nd))
            sn[str(i)] = ls
        eigned_vct = nx.eigenvector_centrality(nx.DiGraph(sn), max_iter = 1000)
        for i in range(self.sub.nodes):
            self.eigen.append(eigned_vct[str(i)])
    ...
    def generate_feature_matrix(self):
        ...
        self.calc_eigen()
        self.calc_degree()
        self.normalize_deg_eigen()

        feature_matrix=[self.crb,self.avail_bandwidth,self.node_max_list,self.band_max_list,self.degree,self.eigen]
        # feature_matrix=[self.crb,self.avail_bandwidth,self.degree,self.eigen]
...
class FeatureVNR:
    ...
    def calc_eigen(self):
        ...
        for i in range(self.vnr.nodes):
            ...
            for nd in self.vnr.neighbours[i]:
                ls.append(int(nd))
            sn[str(i)] = ls
        eigen_vect=nx.eigenvector_centrality(nx.Graph(sn),max_iter=1000)
        for i in range(self.vnr.nodes):
            self.eigen.append(eigen_vect[str(i)])
    ...
    def generate_feature_matrix(self):
        ...
        feature_matrix=[self.crb,self.bandwidth,self.degree,self.eigen]
        feature_matrix = torch.tensor(feature_matrix,dtype=torch.float32)
...
class ActorCritic(nn.Module):
    ...
    def gcn_part(self,vnr,j):
        if(j==0):
            self.virt_graph = self.vnr_gcn(vnr)      # vnr nodes x 128
...
def eigen():
    ...
    actor_critic_model.load_state_dict(torch.load('gcn_rl_eigen_with_congestion.pth'))
