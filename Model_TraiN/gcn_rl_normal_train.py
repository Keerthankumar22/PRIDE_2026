        ...
        if(sub_graph.node_weights[snode] >= vnr_graph.node_weights[vnode] and snode!=-1):
            ...
            for virt_node in range(vnr.x.shape[0]):
                if(vnr.mask[virt_node] != -1 and str(virt_node) in vnr_graph.neighbours[vnode] and virt_node!=vnode):
                    ...
                    paths = sub_graph.findPathFromSrcToDstCongestion(str(sub1),str(sub2),vweight)
                    if(len(paths)!=0):
                        path = paths[0]
                        # print("Path=",path)
...
class GCNNBlock(torch.nn.Module):
    ...
    def setup_layers(self):
        ...
        self.gcnn_layer = ChebConv(self.in_features, 16, 2)  # in, out, k-hops
        self.init_weights()
...
class ActorCritic(nn.Module):
    ...
    def __init__(self,num_sub):
        ...
        self.subs_gcn=GCNNBlock(6)
        # self.vnr_gcn=GCNNBlock(4)
        self.vnr_gcn = Linear(4,16)
        self.policy = Linear(2*16*num_sub,num_sub)
        self.value_head = Linear(2*16*num_sub, 1)
    ...
    def gcn_part(self,vnr,j):
        if(j==0):
            self.virt_graph = self.lrelu(self.vnr_gcn(vnr.x))      # vnr nodes x 128
    ...
    def forward(self,state,j):
        ...
        fully_conn = self.dropout(fully_conn)
...
class Agent(mp.Process):
    ...
    def run(self):
        print("inside gcn_normal agent")
        while(self.episode_id.value < self.total_episodes):
            # self.substrate = extractSubstrate('4_uniform.pickle')       # resetting the environment
            self.substrate = copy.deepcopy(self.first_substrate)
            print("Episode number = ",self.episode_id.value)
            # if self.episode_id.value%50 == 0:
            #     torch.save(self.global_network.state_dict(), f'C:/GCN_Congestion_Aware/all_model_test/cyan5_gcn_rl{400+self.episode_id.value}.pth')

            self.clear_memory()

            # Getting VNRs from create_vne file based on req_no
            req_no = random.randint(30,60)
            vne_list = create_vne(no_requests = req_no)
            vne_list = sorted(vne_list,key=lambda vne:(sum(vne.node_weights.values()) + sum(vne.edge_weights.values())//2))
    ...
    def calc_loss(self, done):
        ...
        for i in range(len(self.actions)):
            prob.append(self.probs[i][self.actions[i]])
